...
var addon = require("../build/".concat((0, _utilsTs.getAddonReleaseVersion)(), "/plugin-rotate-napi.node"));
function rotate(degrees, resize, cb) {
  ...
  if (typeof resize !== "boolean" && typeof resize !== "string") {
    return _utils.throwError.call(this, "Resize must be of type Boolean or String", cb);
  }
  try {
    ...
    if (!resize) {
      addon.advancedRotate(this.bitmap.data, this.bitmap.width, this.bitmap.height, degrees, (0, _utilsTs.cppCallbackWrapper)(this, cb));
      return this;
    }
    ...
    if (degrees % 90 === 0) {
      ...
      addon.matrixRotate(originalBitmap, width, height, newBuffer, newWidth, newHeight, degrees, (0, _utilsTs.cppCallbackWrapper)(this, cb));
      return this;
    }
    ...
    var newWidthHeight = (0, _utilsTs.ensureInteger)(Math.max(width, height, originalImage.getWidth(), originalImage.getHeight()));
    ...
    if (typeof self.crop !== "function") {
      return _utils.throwError.call(self, new Error("A version of plugin-crop must be loaded in order to use resizing with rotate"), cb);
    }
    if (typeof self.blit !== "function") {
      return _utils.throwError.call(self, new Error("A version of plugin-blit must be loaded in order to use resizing with rotate"), cb);
    }
    if (cb) {
      // If we're running in async mode then we'll need to chain these properly through callbacks.
      var doCrop = function doCrop() {
        self.crop((0, _utilsTs.ensureInteger)(self.bitmap.width / 2 - width / 2), (0, _utilsTs.ensureInteger)(self.bitmap.height / 2 - height / 2), width, height, function (err) {
          if (err) {
            return _utils.throwError.call(self, err, cb);
          }
      ...
      self.blit(originalImage, (0, _utilsTs.ensureInteger)(self.bitmap.width / 2 - originalImage.getWidth() / 2), (0, _utilsTs.ensureInteger)(self.bitmap.height / 2 - originalImage.getHeight() / 2), 0, 0, originalImage.getWidth(), originalImage.getHeight(), function (err) {
        if (err) {
          return _utils.throwError.call(self, err, cb);
        }
        doRotate();
      ...
      self.blit(originalImage, (0, _utilsTs.ensureInteger)(self.bitmap.width / 2 - originalImage.getWidth() / 2), (0, _utilsTs.ensureInteger)(self.bitmap.height / 2 - originalImage.getHeight() / 2), 0, 0, originalImage.getWidth(), originalImage.getHeight());
      addon.advancedRotate(self.bitmap.data, self.bitmap.width, self.bitmap.height, degrees);
      self.crop((0, _utilsTs.ensureInteger)(this.bitmap.width / 2 - width / 2), (0, _utilsTs.ensureInteger)(this.bitmap.height / 2 - height / 2), width, height);
    }
  } catch (err) {
...
var plugin = function plugin() {
  return {
    rotate: rotate,
    rotateAsync: (0, _utilsTs.wrapAsync)(rotate)
  };
};
var _default = plugin;
