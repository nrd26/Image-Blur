...
var addon = require("../build/".concat(getAddonReleaseVersion(), "/plugin-crop-napi.node"));
var crop = function crop(x, y, w, h, _cb) {
  ...
  try {
    ...
    if (prependUpdate) {
      ...
      _cb = function cb(err) {
        if (err) {
          return throwError.call(self, err, _cb);
        }
        updateBitmap();
        originalCallback.call(self, null, self);
      };
    }
    addon.crop(self.bitmap.data, self.getWidth(), self.getHeight(), x, y, w, h, cppCallbackWrapper(self, _cb));
    if (!prependUpdate) {
      updateBitmap();
    }
    return self;
  } catch (err) {
    return throwError.call(this, err, _cb);
  }
};
var autocrop = function autocrop() {
  ...
  try {
    addon.autocrop(this.bitmap.data, this.getWidth(), this.getHeight(), ensureInteger(leaveBorder), tolerance, cropOnlyFrames, symmetric, north, east, south, west, this.bitmap, cppCallbackWrapper(this, cb));
    if (!prependUpdate) {
      updateBitmap();
    }
    return this;
